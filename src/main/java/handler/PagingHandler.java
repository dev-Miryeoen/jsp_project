...
public class PagingHandler {
	...
	private int realEndPage; // 제일 마지막 페이지
	...
	public PagingHandler(PagingVO pgvo, int totalCount) {
		...
		// page 1~10 => 10
		// page 11~20 => 20
		// pageNo / 10 => 0.1 0.2 0.3 ... 올림 1 * 10
		this.endPage = (int)(Math.ceil((double)this.pgvo.getPageNo() / 10) * 10);
		this.startPage = this.endPage - 9;
		
		// realEndPage
		// 전체 게시글 수 / 한 페이지에 표시되는 게시글 수 qty => 올림
		// 소수점의 값이 남으면 무조건 한 페이지가 더 필요
		this.realEndPage = (int)Math.ceil((double)totalCount / this.pgvo.getQty());
		
		// 실제 리얼 마지막 페이지가 21페이지라면 1~10 11~20 21
		// endPage 변경
