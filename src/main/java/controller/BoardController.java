...
public class BoardController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		log.info("BoaradController Service method in Test!!");
					...
					case "imageFile" :
						...
						if(item.getSize() > 0) {
							...
							try {
								...
								// 썸네일 작업
								// list 페이지에서 트래픽 과다 사용 방지 (연결 시간지연 방지)
								// 이미지만 가능
...
//		rdp.forward(request, response);
		...
		// forward : 처리 주체가 WAS(톰캣) => 처리 속도가 빠름
		// 	=> 처음 요청 URL이 변경되지 않음. 
		//		/brd/insert 고정되어서 새로고침시 계속 추가
		//		request.getRequestDispatcher("list").forward(request, response);
		
		// redirect : 처리 주체가 Web Browser (was에서 받은 response로 새로운 URL로 다시 Request)
		// 	=> 요청에 따라 다른 URL로 변경됨.
		// 	=> response.sendRedirect("list");
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// Get으로 오는 요청에 대한 처리를 하고, HTML 파일을 생성하여 response 객체를 생성하여 jsp로 전송
