		...
		<main>
			<div class="detail-container row">
				<div class="detail-left col-md-9">
					<h1>
						<span><span class="detail-span-no">No.${b.bno }</span><span class="detail-span-title">${b.title }</span></span>
					...
					<div class="detailBtnBox">
						<a href="/brd/list"><button class="btn btn-dark">목록으로</button></a>
						<c:if test="${ses.id ne null && ses.id == b.writer }">
							<a href="/brd/modify?bno=${b.bno }"><button class="btn btn-primary">수정</button></a>
							<a href="/brd/delete?bno=${b.bno }"><button class="btn btn-danger">삭제</button></a>
						</c:if>
					</div>
				</div>
				<div class="detail-right col-md-3">
					<div class="commentBox">
						...
						<div class="commentInputBox">
							<c:choose>
								<c:when test="${ses.id ne null}">
									<input type="text" id="cmtWriter" class="form-control" placeholder="작성자" value="${ses.id }" readonly="readonly">
								</c:when>
								<c:otherwise>
									<input type="text" id="cmtWriter" class="form-control" placeholder="작성자명">
								</c:otherwise>
							</c:choose>
							<input type="text" id="cmtText" class="form-control" placeholder="댓글">
							<div class="cmtBtnBox">
